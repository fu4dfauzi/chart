...
def BlockSub(request, pk):
	...
	try:
		test = BlockUser.objects.get(blocker = blocker).blocked.get(blocked_u__blocked=blocked)
		...
		obj.save
		messages.info(request, "User is blocked and will not be able to send you messages.")
