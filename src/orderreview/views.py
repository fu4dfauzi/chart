...
class OrderCompleteConfirm(LoginRequiredMixin, GetDetailsMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        new_msg = Message.objects.get_or_create(
            ...
            title=parent_title,
            re_opening=opening,
            content="COMPLETED: This tuition task has been completed by " + str(teacher.user) + " and pending review from "+ str(student.user),
            msgtype="Completed",
            mainmessage=False,
            parent_id=parent_id,
            )[0]
...
class OrderCancelReview(LoginRequiredMixin, GetDetailsMixin, FormView, TemplateView):
    ...
    def form_valid(self, form):
        ...
        new_msg = Message.objects.get_or_create(
            ...
            title=parent_title,
            re_opening=opening,
            content="CANCELED: This tuition task has been canceled by " + str(student.user),
            msgtype="Canceled",
            mainmessage=False,
            parent_id=parent_id,
            )[0]
...
class OrderCompleteReview(LoginRequiredMixin, GetDetailsMixin, FormView, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["title"] = "Please review the tuition job done by " + str(self.get_teacher()) + " for " + str(self.get_opening())
        return context
    ...
    def form_valid(self, form):
        ...
        new_msg = Message.objects.get_or_create(
            ...
            title=parent_title,
            re_opening=opening,
            content="REVIEWED: This tuition task has reviewed by " + str(student.user),
            msgtype="Reviewed",
            mainmessage=False,
            parent_id=parent_id,
            )[0]
